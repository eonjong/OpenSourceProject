...
def first_page(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                result = run(weights='yolov5/runs/train/pet_yolov5s_results/weights/best.pt',
                             source=upload_image_path, imgsz=(640, 640), conf_thres=0.5)
...
def second_page(request):
    ...
    try:
        ...
        # 한국어로 변경한 값
        detected_class_kor = translated_classes.get(
            detected_class, "번역할 수 없는 값")
    except (KeyError, IndexError):
        # 만약 에러가 발생시 에러 페이지로 redirect(연결)
        return redirect('error_page')

    relative_path = uploaded_image.replace(
        settings.MEDIA_ROOT, '').replace('\\', '/')
    image_path = '/'.join([settings.MEDIA_URL.rstrip('/'),
    ...
    # 파일을 읽은 후에 파일을 닫아도 되기 때문에 with 블록 종료 후에 응답을 생성
        response = FileResponse(io.BytesIO(
            audio_content), content_type='audio/mp3')
